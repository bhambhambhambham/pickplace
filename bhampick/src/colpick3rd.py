...
class cr3pick:
    def __init__(self):
        ...
        self.cartesian3 = False
        self.cartesian4 = False
        self.s = rospy.Service("pick", Trigger, self.callback)
        self.s2 = rospy.Service("place", Trigger, self.callback2)
    ...
    def gripperandbox(self, checkbool):
        if checkbool == true:
    ...
    def callback2(self, data2):
        self.setsuccess = False
        self.tf_listener1.waitForTransform("base_link", "place_approach", rospy.Time(), rospy.Duration(1.0))
        (trans, rot) = self.tf_listener1.lookupTransform("base_link", "place_approach", rospy.Time(0))
        ap = Pose()
        ...
        if self.placesuccess == True:
            self.tf_listener2.waitForTransform("base_link", "place_place", rospy.Time(), rospy.Duration(1.0))
            (trans3, rot3) = self.tf_listener2.lookupTransform("base_link", "place_place", rospy.Time(0))
            ...
            self.cartesian3 = self.move_group.execute(plan, wait=True)
            cartesianpath = []
            
            self.gripperandbox(false)
            self.set_home_walkie(0,0,0,0,0,0)
            
            if not self.setsuccess:
                self.tf_listener2.waitForTransform("base_link", "place_safe_retreat", rospy.Time(), rospy.Duration(1.0))
                (trans4, rot4) = self.tf_listener2.lookupTransform("base_link", "place_safe_retreat", rospy.Time(0))
                
                gp.position.x = trans4[0]
                gp.position.y = trans4[1]
                gp.position.z = trans4[2]
                gp.orientation.x = rot4[0]
                gp.orientation.y = rot4[1]
                gp.orientation.z = rot4[2]
                gp.orientation.w = rot4[3]
                
                cartesianpath = []
                cartesianpath.append(copy.deepcopy(gp))
                (plan,fraction) = self.move_group.compute_cartesian_path(cartesianpath, 0.01, 0.0)
                self.cartesian4 = self.move_group.execute(plan, wait=True)
                cartesianpath = []
