...
{
  ros::init(argc, argv, "pickcaller");

  ros::NodeHandle n;
  ros::ServiceClient client = n.serviceClient<bhampick::posedim>("pickit");
  bhampick::posedim srv;
  srv.request.posercam.header.frame_id = "base_link";
  srv.request.posercam.header.stamp = ros::Time::now();
  srv.request.posercam.pose.position.x = 0.6;
  srv.request.posercam.pose.position.y = 0;
  srv.request.posercam.pose.position.z = 1.1;
  ...
  srv.request.dimension.x = 0.0727;
  srv.request.dimension.y = 0.0727;
  srv.request.dimension.z = 0.23;

  if (client.call(srv))
  {
    ROS_INFO("Pick Success : %s", srv.response.graspsuccess ? "true" : "false");
  }
  else
  {
