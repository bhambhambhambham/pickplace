...
class tffrompose{
    ...
    public:
        ...
        void setdimandyaw(const double r, const double p, const double y)
        {
            ...
            try {
                listener2.waitForTransform("world", "mainobj", ros::Time(0), ros::Duration(1.0));
                }
            catch (tf::TransformException& ex) 
            {
                ROS_ERROR("%s", ex.what());
                ...
                if(dimx > gripperlength && dimy > gripperlength){
                    ...
                    if(ay <= 0){
                        if(((yaw < 0 && yaw > -pi/2)&&(ax > fabs(ay)))||((yaw > pi/2 && yaw < pi)&&(ax > fabs(ay)))){
                            ...
                            createCollisionObjectFrameRPY(bc10, "obj", "obj_grasp_poseaaa", 0, 0, dimz/2, 0, pi/2+pi, pi/2);
                            createCollisionObjectFrameRPY(bc11, "obj", "obj_approach_poseaaa", 0, 0, dimz/2+0.05, 0, pi/2+pi, pi/2);
                            createCollisionObjectFrameRPY(bc12, "obj", "obj_lift_poseaaa", 0, 0, dimz/2+0.05, 0, pi/2+pi, pi/2);
                        }
                        else{
                            createCollisionObjectFrameRPY(bc1, "obj", "obj_grasp_pose", 0, dimy/2, 0, 0, 0, -pi/2);
                            createCollisionObjectFrameRPY(bc2, "obj", "obj_approach_pose", 0, dimy/2+0.05, 0, 0, 0, -pi/2);
