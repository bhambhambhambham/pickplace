...
class tffrompose{
    ...
    public:
        tffrompose(ros::NodeHandle *nodehan){
            subscriber = nodehan->subscribe("/custompose", 10, &tffrompose::callback1,this);
        ...
        void callback1(const geometry_msgs::PoseStamped& data){
            ...
            createCollisionObjectFrame(bcmain, "world", "mainobj", ax, ay, az, qx, qy, qz, qw);
        ...
        {
            dimdatax = data.dimensionx;
            dimdatay = data.dimensiony;
            dimdataz = data.dimensionz;
            if(check == true)
            {
                addCollisionObjects(planning_scene_interface, "world", "box1", ax, ay, az, qx, qy, qz, qw, dimx, dimy, dimz);
            }
        }
        ...
        { 
            ...
            primitive.dimensions[0] = dimdatax;
            primitive.dimensions[1] = dimdatay;
            primitive.dimensions[2] = dimdataz;
        ...
        void setdimandyaw(const double r, const double p, const double y)
        {
            ...
            if(y > pi/2 || y < -pi/2){
                createCollisionObjectFrameRPY(bcobj, "world", "obj", ax, ay, az, 0, 0, y+pi);
            }
            else{
                createCollisionObjectFrameRPY(bcobj, "world", "obj", ax, ay, az, 0, 0, y);
            }
            return;
        }
