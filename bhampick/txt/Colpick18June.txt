...
class cr3pick:
    ...
    def main(self):
        ...
        boxpose.pose.position.x = 0.5
        boxpose.pose.position.y = 0
        boxpose.pose.position.z = -0.02
        ...
        scene.add_box("Col1", boxpose, size=(1, 2, 0.02))
        
        boxpose2 = PoseStamped()
        q = tf.transformations.quaternion_from_euler(0, 0, 0)
    
        boxpose2.header.frame_id = "world"
        boxpose2.pose.position.x = -0.125
        boxpose2.pose.position.y = 0
        boxpose2.pose.position.z = 0.3
    
        boxpose2.pose.orientation.x = q[0]
        boxpose2.pose.orientation.y = q[1]
        boxpose2.pose.orientation.z = q[2]
        boxpose2.pose.orientation.w = q[3]
    
        scene.add_box("Col2", boxpose2, size=(0.02, 2, 0.6))
    ...
    def callback(self, data):
        ...
        while (self.success == False):
            ...
            self.tf_listener1.waitForTransform("cr3_base_link", self.subframe1, rospy.Time(), rospy.Duration(1.0))
            (trans, rot) = self.tf_listener1.lookupTransform("cr3_base_link", self.subframe1, rospy.Time(0))
        ...
        if self.success == True:
            self.tf_listener2.waitForTransform("cr3_base_link", self.subframe1, rospy.Time(), rospy.Duration(1.0))
            (trans2, rot2) = self.tf_listener2.lookupTransform("cr3_base_link", self.subframe2, rospy.Time(0))
            ...
            print(self.move_group.execute(plan, wait=True))
            ...
            self.tf_listener3.waitForTransform("cr3_base_link", self.subframe1, rospy.Time(), rospy.Duration(1.0))
            (trans3, rot3) = self.tf_listener3.lookupTransform("cr3_base_link", self.subframe3, rospy.Time(0))
